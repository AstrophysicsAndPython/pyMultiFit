...
class LaplaceFitter(BaseFitter):
    ...
    def fit_boundaries(self):
        """Return the parameter boundaries for the Laplace fitter.

        Returns
        -------
        tuple[tuple, tuple]
            Lower and upper bounds for ``(amplitude, mu, scale)``.
        """
        lb = (0, -np.inf, 0)
    ...
    def fitter(x, params: ArrayLike):
        """Evaluate the Laplace PDF at *x* given *params*.
        ...
        params :
            Parameter array ``[amplitude, mu, scale]``.
