...
class ChiSquareFitter(BaseFitter):
    ...
    def fit_boundaries(self):
        """Return the parameter boundaries for the ChiSquare fitter.

        Returns
        -------
        tuple[tuple, tuple]
            Lower and upper bounds for ``(amplitude, degree_of_freedom, loc)``.
        """
        lb = (0, 0, -np.inf)
    ...
    def fitter(x, params: ArrayLike):
        """Evaluate the ChiSquare PDF at *x* given *params*.
        ...
        params :
            Parameter array ``[amplitude, degree_of_freedom, loc]``.
