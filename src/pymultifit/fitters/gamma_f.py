...
class GammaFitter(BaseFitter):
    ...
    def fit_boundaries(self):
        """Return the parameter boundaries for the Gamma fitter.

        Returns
        -------
        tuple[tuple, tuple]
            Lower and upper bounds for ``(amplitude, alpha, theta, loc)``.
        """
        lb = (0, 0, 0, -np.inf)
    ...
    def fitter(x, params: ArrayLike):
        """Evaluate the Gamma PDF at *x* given *params*.
        ...
        params :
            Parameter array ``[amplitude, alpha, theta, loc]``.
